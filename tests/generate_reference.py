...
def main(dim, res, boxsize, mas_order, slow=False, save=False):
    print(slow)
    ...
    results_norm = BFast.Bk.jit(field, boxsize, mas_order, **B_info_openclosed, fast=not slow, only_B=False, compute_norm=True)
    ...
    results_unnorm = BFast.Bk.jit(field, boxsize, mas_order, **B_info_openclosed, fast=not slow, only_B=False, compute_norm=False)
    print("Pk unnormalized:", results_unnorm['Pk'])
    print("Bk unnormalized:", results_unnorm['Bk'], end='\n\n')

    if save: jnp.savez(f'reference_data/measured_PB_dim{dim}_res{res}_mas{mas_order}_openclosed', **results_unnorm)
...
if __name__ == "__main__":
    ...
    main(args.dim, args.res, args.boxsize, args.mas_order, args.slow, args.save)