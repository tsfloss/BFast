...
ref_meas = jnp.load(f"tests/reference_data/measured_PB_dim{dim}_res{res}_openclosed.npz")
...
def test_norm_unjitted_fast():
    result = BFast.Bk(field, boxsize, **B_info, fast=True, only_B=False, compute_norm=True)
    assert jnp.allclose(result['Pk'], ref_norm['Pk'])
    assert jnp.allclose(result['Bk'], ref_norm['Bk'])

def test_norm_unjitted_slow():
    result = BFast.Bk(field, boxsize, **B_info, fast=False, only_B=False, compute_norm=True)
    assert jnp.allclose(result['Pk'], ref_norm['Pk'])
    assert jnp.allclose(result['Bk'], ref_norm['Bk'])

def test_norm_jitted_fast():
    result = BFast.Bk.jit(field, boxsize, **B_info, fast=True, only_B=False, compute_norm=True)
    assert jnp.allclose(result['Pk'], ref_norm['Pk'])
    assert jnp.allclose(result['Bk'], ref_norm['Bk'])

def test_norm_jitted_slow():
    result = BFast.Bk.jit(field, boxsize, **B_info, fast=False, only_B=False, compute_norm=True)
    assert jnp.allclose(result['Pk'], ref_norm['Pk'])
    assert jnp.allclose(result['Bk'], ref_norm['Bk'])

def test_meas_unjitted_fast():
    result = BFast.Bk(field, boxsize, **B_info, fast=True, only_B=False, compute_norm=False)
    assert jnp.allclose(result['Pk'], ref_meas['Pk'])
    assert jnp.allclose(result['Bk'], ref_meas['Bk'])

def test_meas_unjitted_slow():
    result = BFast.Bk(field, boxsize, **B_info, fast=False, only_B=False, compute_norm=False)
    assert jnp.allclose(result['Pk'], ref_meas['Pk'])
    assert jnp.allclose(result['Bk'], ref_meas['Bk'])

def test_meas_jitted_fast():
    result = BFast.Bk.jit(field, boxsize, **B_info, fast=True, only_B=False, compute_norm=False)
    assert jnp.allclose(result['Pk'], ref_meas['Pk'])
    assert jnp.allclose(result['Bk'], ref_meas['Bk'])

def test_meas_jitted_slow():
    result = BFast.Bk.jit(field, boxsize, **B_info, fast=False, only_B=False, compute_norm=False)
    assert jnp.allclose(result['Pk'], ref_meas['Pk'])
    assert jnp.allclose(result['Bk'], ref_meas['Bk'])