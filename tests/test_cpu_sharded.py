...
def test_norm_unjitted_fast():
    result = BFast.Bk(field, boxsize, mas_order, **B_info, fast=True, only_B=False, compute_norm=True, sharding=sharding)
    assert jnp.allclose(result['Pk'], ref_norm['Pk'])
    assert jnp.allclose(result['Bk'], ref_norm['Bk'])

def test_norm_unjitted_slow():
    result = BFast.Bk(field, boxsize, mas_order, **B_info, fast=False, only_B=False, compute_norm=True, sharding=sharding)
    assert jnp.allclose(result['Pk'], ref_norm['Pk'])
    assert jnp.allclose(result['Bk'], ref_norm['Bk'])

def test_norm_jitted_fast():
    result = BFast.Bk.jit(field, boxsize, mas_order, **B_info, fast=True, only_B=False, compute_norm=True, sharding=sharding)
    assert jnp.allclose(result['Pk'], ref_norm['Pk'])
    assert jnp.allclose(result['Bk'], ref_norm['Bk'])

def test_norm_jitted_slow():
    result = BFast.Bk.jit(field, boxsize, mas_order, **B_info, fast=False, only_B=False, compute_norm=True, sharding=sharding)
    assert jnp.allclose(result['Pk'], ref_norm['Pk'])
    assert jnp.allclose(result['Bk'], ref_norm['Bk'])

def test_meas_unjitted_fast():
    result = BFast.Bk(field, boxsize, mas_order, **B_info, fast=True, only_B=False, compute_norm=False, sharding=sharding)
    assert jnp.allclose(result['Pk'], ref_meas['Pk'])
    assert jnp.allclose(result['Bk'], ref_meas['Bk'])

def test_meas_unjitted_slow():
    result = BFast.Bk(field, boxsize, mas_order, **B_info, fast=False, only_B=False, compute_norm=False, sharding=sharding)
    assert jnp.allclose(result['Pk'], ref_meas['Pk'])
    assert jnp.allclose(result['Bk'], ref_meas['Bk'])

def test_meas_jitted_fast():
    result = BFast.Bk.jit(field, boxsize, mas_order, **B_info, fast=True, only_B=False, compute_norm=False, sharding=sharding)
    assert jnp.allclose(result['Pk'], ref_meas['Pk'])
    assert jnp.allclose(result['Bk'], ref_meas['Bk'])

def test_meas_jitted_slow():
    result = BFast.Bk.jit(field, boxsize, mas_order, **B_info, fast=False, only_B=False, compute_norm=False, sharding=sharding)
    assert jnp.allclose(result['Pk'], ref_meas['Pk'])
    assert jnp.allclose(result['Bk'], ref_meas['Bk'])